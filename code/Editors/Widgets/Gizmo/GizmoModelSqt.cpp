...
namespace Editors
{
	...
	void GizmoModelSqt::SetSqt(Core::Sqt* pSqt)
	{
		if (m_pSqt == pSqt)
			return;
	...
	const Core::Mat44f GizmoModelSqt::GetTransform() const
	{
		...
		Core::Mat44f txWs = m_pSqt->GetMatrix();
	...
	void GizmoModelSqt::Translate(const Core::Vec4f& worldPos)
	{
		...
		m_pSqt->SetTranslation(worldPos);
		//new world transform
	...
	void GizmoModelSqt::Rotate(const Core::Mat44f& rotation)
	{
		...
		Core::Mat44f wsTx = rotation * m_pSqt->GetMatrix();
		*m_pSqt = Core::Sqt(wsTx);
	...
	void GizmoModelSqt::IncrementScale(const Core::Vec4f& scale)
	{
		...
		Core::Vec4f localScale = m_pSqt->GetScale();
		...
		Core::Vec4f newScale = m_pSqt->GetTranslation() * oneOverScale * newLocalScale;

		m_pSqt->SetScale(newScale);
