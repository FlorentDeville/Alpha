...
namespace Systems
{
	static void Internal_Bind(const Rendering::PerObjectCBuffer& perObjectCBuffer,
					const Rendering::PerFrameCBuffer& perFrameCBuffer,
					const Rendering::LightsArrayCBuffer& lights,
					const Core::Array<MaterialCBufferBindingInfo>& bindingInfoArray,
					const Core::Array<Systems::MaterialParameterDescription>& perMaterialParam,
					const Core::Array<TextureBindingInfo>& textureBindingInfo,
					const Rendering::PipelineState& pso, const Rendering::RootSignature& rs)
	{
		Rendering::RenderModule& renderer = Rendering::RenderModule::Get();

		renderer.BindMaterial(pso, rs);
	...
	void MaterialRendering::Bind(const MaterialAsset& material, const Rendering::PerObjectCBuffer& perObjectCBuffer, 
		...
		const Rendering::LightsArrayCBuffer& lights)
	{
		Internal_Bind(perObjectCBuffer, perFrameCBuffer, lights, material.GetBindingInfoArray(), material.GetMaterialParameterDescription(),
			material.GetTexturesBindingInfo(), *material.GetPipelineState(), *material.GetRootSignature());
	}
